...
const DialogueText = styled.div<{ $system: boolean }>`
  font-size: ${({ $system }) => ($system ? '2rem' : '1.5rem')};
  color: ${({ $system }) => ($system ? '#FFF583' : 'white')};
  line-height: 1.4;
  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5);
  font-family: ${({ $system }) =>
    $system ? "'Cafe24ClassicType'" : 'inherit'};
`;
...
const PrologPage = () => {
  ...
  const dialogues = [
    { speaker: 'system', text: "오늘도 평범하게 등교를 하는 미림이" },
    { speaker: 'system', text: "오늘 하루도 학교를 폭파시키고 싶다는 생각으로 가득 차 있다." },
    { speaker: 'player', text: "오늘도 등교, 내일도 등교.. 오늘도 학교를 폭파시키고 싶다..!" },
  ] as const;
  ...
  const isSystem = currentDialogue.speaker === 'system';
  ...
  return (
    <Container $bg={background5} onClick={handleScreenClick}>
      ...
      {step === 1 && (
        <IntroOverlay>
          <IntroText>샤랄라 불만이 쌓이는 아침</IntroText>
        </IntroOverlay>
      )}
          ...
          <DialogueSection>
            {isSystem && (
              <ProfileWrapper>
                <ProfileInner>
                  <ProfileImage
                    src={speakerConfig.system.profile || ''}
                    alt="Profile"
                  />
                </ProfileInner>
              </ProfileWrapper>
            ...
            <MessageBox>
              <NameTag>{speakerConfig[currentDialogue.speaker].name}</NameTag>
              <DialogueText $system={isSystem}>
